...
def remove_cart(request: HttpRequest, product_id, cart_item_id):
...
def remove_cart_item(request: HttpRequest, product_id, cart_item_id):
    product = get_object_or_404(Product, id=product_id)
    if request.user.is_authenticated:
        cart_item: CartItem = CartItem.objects.get(
            product=product, user=request.user, id=cart_item_id
...
def checkout(request: HttpRequest, total=0, quantity=0, cart_items=None):
    try:
        tax = 0
        grand_total = 0
